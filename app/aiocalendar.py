...
ua_month_name = {'December': 'Грудень',
                 'January': 'Січень',
                 'February': 'Лютий',
                 'March': 'Березень',
                 'April': 'Квітень',
...
def create_calendar(year=datetime.now().year, month=datetime.now().month, now_day=datetime.now().day):
    """
    ...
    inline_kb.insert(
        InlineKeyboardButton(f"{ua_month_name[calendar.month_name[month]]}", callback_data=ignore_callback))
